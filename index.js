...
export default function(str) {
  ...
  var scenex = {
    ...
    type:       /CAM|\bTS(?!C)\b|TELESYNC|(DVD|BD)SCR|\bSCR\b|DDC|R5([\.\s\-]LINE)?|(DVD|HD|BR|BD|WEB)Rip|DVD(R)?|(HD|PD)TV|BluRay|(WEB(-)?(DL)?)/i,
    audio:      /(AAC([0-9][\.\s][0-9])?|AAC)|(DTS([\.\s\-](HD([\.\s\-]MA)?))?(([\.\s\-])?\d[\.\s\-]\d)?)|(AC3([\.\s](DD([0-9][\.\s][0-9])?)?)?)|\b(DD(P)?(([\+\-\.\s])?([\+\-\.\s]ATMOS)?(\d)?[\-\.\s]\d)?([\-\.\s]ATMOS)?)\b|((ATMOS[\.\-\s])?TRUE([\-\.\s])?HD[\.\-\s](ATMOS[\.\-\s])?(\d[\.\-\s]\d)?|(ATMOS)([\.\s\-]\d[\.\s\-]\d)?)/i,
    other:      /UHD|DOCU/i,
    group:      /\w+$/i
  };
